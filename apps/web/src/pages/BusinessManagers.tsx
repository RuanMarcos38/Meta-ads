...
const tokenLabel=(value?:string)=>value==='valid'?'válido':value==='expired'?'expirado':value==='invalid'?'inválido':value==='unknown'?'não verificado':value||'não verificado';
const syncLabel=(value?:string)=>value==='success'?'concluída':value==='running'?'em andamento':value==='error'?'com erro':value==='pending'?'pendente':value||'nunca';
...
export default function BusinessManagers() {
  ...
  async function load() {
    ...
    } catch (e: any) { setError(e?.response?.data?.error?.message || 'Não foi possível carregar os Gerenciadores de Negócios.'); }
  ...
  async function refresh() {
    ...
    try {
      ...
      await load();
    } catch (e: any) { setError(e?.response?.data?.error?.message || 'Não foi possível atualizar os Gerenciadores de Negócios pela Meta.'); }
  ...
  async function sync(manager: Manager, fullHistory = false) {
    ...
    catch (e: any) { setError(e?.response?.data?.error?.message || 'Não foi possível sincronizar este Gerenciador de Negócios.'); }
    finally { setSyncing(''); }
  }
  ...
  return <div className="space-y-4">
    <section className="page-heading"><div><p className="section-kicker">Estrutura Meta</p><h1>Gerenciadores de Negócios</h1><p>Cada Gerenciador de Negócios fica isolado por empresa, com contas, token, histórico e sincronização próprios.</p></div>{isAdmin && <button className="primary-button" onClick={() => { void refresh(); }} disabled={refreshing}><RefreshCw size={14} className={refreshing ? 'animate-spin' : ''} />{refreshing ? 'Consultando a Meta' : 'Atualizar gerenciadores'}</button>}</section>
    {error && <div className="message-warning">{error}</div>}
    <section className="grid gap-3 xl:grid-cols-2">{displayRows.map((row) => { const h = healthMap.get(row.metaBusinessId); return <article key={row.id} className="corporate-card p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-[7px] bg-[#eff6ff] text-[#2563eb]"><BriefcaseBusiness size={17} /></span><div className="min-w-0"><h2 className="truncate text-[14px] font-semibold">{row.name}</h2><p className="mt-0.5 text-[10px] text-slate-400">ID do Gerenciador: {row.metaBusinessId}</p><p className="mt-0.5 text-[10px] text-slate-500">Empresa: {row.client?.name || h?.clientName || row.clientId}</p></div></div><span className={`status-chip ${h?.connected && h.tokenStatus === 'valid' ? 'status-success' : h?.connected ? 'status-warning' : 'status-neutral'}`}>{h?.connected ? <ShieldCheck size={12} /> : <ShieldAlert size={12} />}{h?.connected ? `Meta · token ${tokenLabel(h.tokenStatus)}` : 'Desconectado'}</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div className="mini-stat"><span>Contas</span><strong>{h?.assignedAccountCount ?? row._count?.adAccounts ?? 0}</strong><small>autorizadas</small></div><div className="mini-stat"><span>Histórico</span><strong>{h?.dataRows?.toLocaleString('pt-BR') || '0'}</strong><small>linhas no banco</small></div><div className="mini-stat"><span>Última sincronização</span><strong>{h?.lastSyncAt ? new Date(h.lastSyncAt).toLocaleDateString('pt-BR') : '—'}</strong><small>{syncLabel(h?.lastSyncStatus)}</small></div><div className="mini-stat"><span>Período</span><strong>{h?.earliestDate ? new Date(h.earliestDate).toLocaleDateString('pt-BR') : '—'}</strong><small>até {h?.latestDate ? new Date(h.latestDate).toLocaleDateString('pt-BR') : '—'}</small></div></div><div className="mt-4 border-t border-[#e3e8e5] pt-3"><label className="field-label">E-mail administrativo do Gerenciador<input defaultValue={row.adminEmail || ''} onBlur={(e) => { if (isAdmin && e.target.value !== (row.adminEmail || '')) void saveEmail(row,e.target.value); }} readOnly={!isAdmin} placeholder="responsavel@empresa.com.br" className="field-control mt-1 max-w-md" /></label></div>{h?.lastError && <div className="message-warning mt-3">{h.lastError}</div>}<div className="mt-3 flex flex-wrap items-center gap-2"><button className="secondary-button" disabled={syncing === row.id} onClick={() => { void sync(row,false); }}><RefreshCw size={13} className={syncing === row.id ? 'animate-spin' : ''} />Sincronizar agora</button><button className="secondary-button" disabled={syncing === row.id} onClick={() => { void sync(row,true); }}><Clock3 size={13} />Importar histórico</button><span className="ml-auto text-[9px] text-slate-400">{h?.tokenExpiresAt ? `Token válido até ${new Date(h.tokenExpiresAt).toLocaleDateString('pt-BR')}` : ''}</span></div></article>; })}{!displayRows.length && !loading && <div className="empty-state corporate-card col-span-full"><Wrench size={20} /><span>Nenhum Gerenciador de Negócios persistido ainda. Use “Atualizar gerenciadores” após conectar a Meta.</span></div>}</section>
  </div>;
}
