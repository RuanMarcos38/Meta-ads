  ...
  return <div className="space-y-4">
    <section className="page-heading">
      <div>
        ...
        <h1>Empresas</h1>
        <p>Uma linha por empresa. BMs, contas, usuários, integração e histórico ficam agrupados dentro da empresa.</p>
      </div>
    ...
    {canAdmin && editing && <section className="corporate-card p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="section-kicker">Editar empresa</p>
          <h2 className="text-lg font-semibold">{editing.name}</h2>
          <p className="text-sm text-slate-500">Atualize somente os dados cadastrais. BMs, contas, usuários e integrações permanecem vinculados normalmente.</p>
        </div>
      ...
      <form onSubmit={saveEdit} className="space-y-3">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="space-y-1"><span className="text-xs font-medium text-slate-600">E-mail</span><input type="email" className="field-control w-full" value={editForm.email} onChange={(e) => setEditForm((v) => ({ ...v, email: e.target.value }))} /></label>
          <label className="space-y-1"><span className="text-xs font-medium text-slate-600">Telefone</span><input className="field-control w-full" value={editForm.phone} onChange={(e) => setEditForm((v) => ({ ...v, phone: e.target.value }))} /></label>
    ...
    {visible.map((client) => expanded === client.id && <section key={`${client.id}-detail`} className="corporate-card overflow-hidden">
