...
export default function SupportPro(){
 ...
 return <div className="space-y-4">
  <section className="page-heading">
   <div><p className="section-kicker">Comunicação interna</p><h1>Atendimento</h1><p>Chat ao vivo entre administradores e usuários da mesma empresa, com chamados, presença, arquivos e áudio.</p></div>
  ...
  <section className="grid min-h-[620px] overflow-hidden rounded-[8px] border border-[#dfe4e1] bg-white lg:grid-cols-[330px_1fr]">
   <aside className="border-b border-[#e1e6e3] lg:border-b-0 lg:border-r">
    ...
    <div className="premium-scrollbar max-h-[500px] overflow-y-auto border-t border-[#e5e9e6]">{visible.map(c=><button key={c.id} onClick={()=>setSelectedId(c.id)} className={`block w-full border-b border-[#edf0ee] p-3 text-left ${selectedId===c.id?'bg-[#eef4ff]':'hover:bg-[#fafbfa]'}`}><div className="flex items-center justify-between gap-2"><strong className="truncate text-[11px]">{c.type==='CHAT'?(c.peer?.name||c.subject||'Atendimento'):c.subject||'Chamado'}</strong><span className="text-[8px] text-slate-400">{dt(c.lastMessageAt)}</span></div><p className="mt-1 truncate text-[9px] text-slate-500">{c.lastMessage?.body||c.lastMessage?.attachmentName||c.peer?.email||'Conversa iniciada'}</p><div className="mt-2 flex items-center gap-1"><span className="status-chip status-neutral">{c.type==='TICKET'?<TicketCheck size={10}/>:<MessageCircle size={10}/>} {c.status}</span>{c.unread&&<span className="status-chip status-danger">Nova</span>}{c.slaBreached&&<span className="status-chip status-warning">SLA</span>}</div></button>)}</div>
   </aside>

   <main className="flex min-w-0 flex-col">{selected?<>
    <header className="flex flex-wrap items-center justify-between gap-2 border-b border-[#e1e6e3] p-4"><div><h2 className="text-[13px] font-semibold">{selected.type==='CHAT'?(selected.peer?.name||selected.subject||'Atendimento interno'):selected.subject||'Chamado'}</h2><p className="mt-1 text-[9px] text-slate-500">{selected.peer?.email?`${selected.peer.email} · `:''}{selected.type==='TICKET'?`Prioridade ${selected.priority}`:'Conversa interna'} · {selected.status}</p></div>{admin&&<div className="flex gap-1">{selected.type==='TICKET'&&<button className="secondary-button" onClick={()=>{void update({assignToMe:true});}}><UserRoundCheck size={12}/>Assumir</button>}<select className="field-control w-auto" value={selected.status} onChange={e=>{void update({status:e.target.value});}}><option value="OPEN">Aberto</option><option value="PENDING">Aguardando</option><option value="RESOLVED">Resolvido</option><option value="CLOSED">Encerrado</option></select></div>}</header>
    <div className="premium-scrollbar flex-1 space-y-3 overflow-y-auto bg-[#f8fafc] p-4">{msgs.map(m=>{const mine=m.senderId===user?.id;return <div key={m.id} className={`flex ${mine?'justify-end':'justify-start'}`}><div className={`max-w-[82%] rounded-[8px] border px-3 py-2 ${mine?'border-[#bfdbfe] bg-[#eff6ff]':'border-[#e0e5e2] bg-white'}`}><div className="mb-1 flex items-center gap-2 text-[8px] text-slate-400"><span>{m.sender?.name||'Usuário'}</span><span>{dt(m.createdAt)}</span></div>{m.body&&<p className="m-0 whitespace-pre-wrap text-[11px] leading-5">{m.body}</p>}{m.attachmentName&&<button className="mt-2 flex items-center gap-2 text-[10px] font-semibold text-[#2563eb]" onClick={()=>{void open(m);}}>{m.kind==='AUDIO'?<Headphones size={13}/>:<FileText size={13}/>} {m.attachmentName}</button>}{m.kind==='AUDIO'&&audioUrls[m.id]&&<audio controls src={audioUrls[m.id]} className="mt-2 max-w-full"/>}</div></div>})}<div ref={bottom}/></div>
    <footer className="border-t border-[#e1e6e3] bg-white p-3">{attachment&&<div className="mb-2 flex items-center justify-between rounded-[6px] bg-[#f2f5f3] px-3 py-2 text-[9px]"><span>{attachment.kind==='AUDIO'?'Áudio':'Arquivo'}: {attachment.name}</span><button onClick={()=>setAttachment(null)}><X size={13}/></button></div>}<div className="flex items-end gap-2"><textarea className="field-control min-h-[42px] flex-1" placeholder="Digite uma mensagem..." value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();void send();}}}/><label className="icon-button cursor-pointer"><Paperclip size={14}/><input type="file" className="hidden" onChange={e=>{void choose(e.target.files?.[0]);e.target.value='';}}/></label><button className={`icon-button ${recording?'text-red-600':''}`} onClick={()=>{void toggleRecord();}}>{recording?<Square size={13}/>:<Mic size={14}/>}</button><button className="primary-button h-[36px]" disabled={sending||(!draft.trim()&&!attachment)} onClick={()=>{void send();}}><Send size={14}/></button></div></footer>
